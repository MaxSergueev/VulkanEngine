...
void VulkanEngine::init_vulkan()
{
    ...
    auto inst_ret = builder.set_app_name("Vulkan Application Test")
    ...
    while (!bQuit)
    {
        ...
        if (showGui) {
            if (ImGui::Begin("Gray-Scott Reaction Diffusion")) {
                ...
                if (ImGui::Button("Mitosis (Dividing Cells)")) {
                    // Spots that grow and divide continually
                    selected.data.data2 = glm::vec4(1.0f, 0.45f, 0.036f, 0.065f);
                    selected.data.data3.x = 1.0f;
                }
                ImGui::SameLine();
                if (ImGui::Button("Solitons (Moving Bugs)")) {
                    // Highly dynamic, moving spots that bounce off each other
                    selected.data.data2 = glm::vec4(1.0f, 0.45f, 0.014f, 0.050f);
                    selected.data.data3.x = 1.0f;
                }
                ImGui::SameLine();
                if (ImGui::Button("Stable Mazes")) {
                    // Slowly forms a static, brain-like labyrinth
                    selected.data.data2 = glm::vec4(1.0f, 0.45f, 0.029f, 0.057f);
                    selected.data.data3.x = 1.0f;
                }

                if (ImGui::Button("Holes")) {
                    // The inverse of spots: empty spaces growing in a solid background
                    selected.data.data2 = glm::vec4(1.0f, 0.45f, 0.039f, 0.058f);
                    selected.data.data3.x = 1.0f;
                }
                ImGui::SameLine();
                if (ImGui::Button("Chaos (Boiling)")) {
                    // An unstable, constantly flickering and morphing texture
                    selected.data.data2 = glm::vec4(1.0f, 0.45f, 0.026f, 0.051f);
                    selected.data.data3.x = 1.0f;
                }
                ImGui::SameLine();
                if (ImGui::Button("Reset Default")) {
                    // Your original expanding coral/spot pattern
